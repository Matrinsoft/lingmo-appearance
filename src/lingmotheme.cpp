    ...
    : QObject(parent)
{
    ...
    _darkMode = static_cast<int>(DarkMode::Light);
    _nativeText = false;
...
{
    if (_darkMode == static_cast<int>(DarkMode::Dark)) {
        return true;
    } else if (_darkMode == static_cast<int>(DarkMode::System)) {
        return _systemDark;
    } else {
        return false;
    }
...
void LingmoTheme::checkUpdateDesktopImage()
{
    ...
    QThreadPool::globalInstance()->start([=]() {
        _mutex.lock();
        // Desktop image path detection will be implemented via DBus
        _mutex.unlock();
    });
}

void LingmoTheme::timerEvent(QTimerEvent*)
{
    checkUpdateDesktopImage();
}
